...
void Drift_correction(void)
{
  ...
  Vector_Cross_Product(&errorRollNick[0],&Accel_Vector[0],&DCM_Matrix[2][0]); //adjust the ground of reference
...
void Euler_angles(void)
{
    ...
    nick = -asin((Accel_Vector[0])/(float)GRAVITY); // asin(acc_x)
    yaw = 0;
  #else        // Euler angles from DCM matrix
    nick = asin(-DCM_Matrix[2][0]);
